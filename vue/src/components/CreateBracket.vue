<template>
    <div>
        ...
        <div>
            <div class= "headers">
                <h3 v-for="Round in this.getRoundTitles" v-bind:key='Round' >{{Round}}</h3>
            </div>
            <div class= "matches">
                <div class= "match" v-for="(match,index) in this.constuctMatches" v-bind:key='match.index'>
                <div class="team-top"><input type="text"/></div>
                <div     v-if="hasOddPants && index != constuctMatches.length-1 "    class="team-bottom"><input type="text"/></div><br>
                </div>
            </div>
        </div>    
